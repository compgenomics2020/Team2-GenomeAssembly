...
forward_score_list = []
reverse_score_list = []
def find_cropping_and_trim(file, qual="25"):
...
# Now, original fastq files are called in pairs to be trimmed by fastp
			with open("all_fastq_files_paired.txt", 'r') as trimming_input:
				...
				quality = str(qual)
				halfway_point = len(forward_score_list) / 2
				head_command = max(forward_score_list[:halfway_point])
				tail_command = min(forward_score_list[halfway_point:])
				print(head_command, tail_command)
				subprocess.call(["./fastp", "--in1", fastqc_file_1, "--in2", fastqc_file_2, "--stdout", "--out1", output_name, "--out2", output_name_2, "--failed_out", fail_file, "-m", "--merged_out", merged_file, "-l", "50", "-q", quality, "-j", output_name_json, "-h", output_name_html, '-w', '10', '-f', head_command, "-t", tail_command])
